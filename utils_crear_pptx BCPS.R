...
grafica_planeacion <- function(df, col_valor, titulo,
                               anio_meta = 2026,
                               beige = "#D9D2BE",
                               verde = "#2F6F63") {
  df %>%
    mutate(anio = as.factor(anio),
           color = ifelse(as.integer(as.character(anio)) == anio_meta, "meta", "hist")) %>%
    ggplot(aes(x = anio, y = .data[[col_valor]], fill = color)) +
    geom_col(width = 0.82) +
    geom_text(aes(label = scales::comma(.data[[col_valor]])),
              vjust = -0.35, fontface = "bold", size = 4) +
    scale_fill_manual(values = c(hist = beige, meta = verde)) +
    ...
    theme(
      ...
      panel.grid = element_blank(),          # <- quita toda la rejilla
...
grafica_consultas_periodos <- function(df,
  ...
  # Puntos destacados: marzo de cada año y último mes visible
  puntos_destacados <- df %>%
    dplyr::filter(lubridate::month(fecha) == mes_a_resaltar) %>%
  ...
  ggplot(df, aes(x = fecha, y = consultas_totales)) +
    ...
    geom_text(
      ...
      aes(
        label = paste0(
          ...
          gsub("\\.", "", stringr::str_to_title(clock::date_format(fecha, format = "%b %Y",  locale = clock::clock_locale("es"))))
        )
      ),
      fontface = "bold",
      size = 3.2,
    ...
    annotate(
      "label",
      x = fecha_ultimo_valor,
      y = ymax * 1.08,
      label = paste0(
        scales::comma(valor_ultimo), "\n",
        stringr::str_to_title(format(fecha_ultimo_valor, "%B %Y"))
      ),
      hjust = 1,
      fill = fill_valuebox,
      color = "white",
    ...
    scale_x_date(
      breaks = "1 month",
      labels = etiquetas_inteligentes,             # Etiquetas en breaks principales
      date_minor_breaks = "1 month", # Mes abreviado + año en nueva línea
      expand = c(0, 0)       # Reduce espacio en los extremos
    ) +
    labs(title = titulo, x = NULL, y = NULL) +
    theme_minimal(base_size = 12) +
    theme(
...
crear_reporte_productividad <- function(
  ...
  fecha_portada <-   clock::date_format(
    Sys.Date(),
    format = "%d de %B de %Y",
    locale = clock::clock_locale("es") # '"es"' sería "es"
  )
  mes_nombre <- stringr::str_to_title(  clock::date_format(
    Sys.Date(),
    format = "%B",
    locale = clock::clock_locale("es") # '"es"' sería "es"
  ))
  ...
  historicos_filtrado <- historicos# %>%
  # dplyr::filter(clues == codigo_clues) |>
  # rename(clues=clues_imb)


  procedimientos_personas_filtrado <- procedimientos_personas %>%
    # dplyr::rename(clues_imb = clues) %>%
    # dplyr::filter(clues_imb == codigo_clues) %>%
    dplyr::mutate(
  ...
  datos_consulta_funcion <- historicos_filtrado %>%
    dplyr::mutate(
      anio=year(fecha),
  ...
  for (col in cols_metricas) {
    for (ref in c(2024, 2025)) {
      ...
      valor_2026 <- datos_consulta_funcion[[col]][datos_consulta_funcion$anio == 2026]
      valor_ref  <- datos_consulta_funcion[[col]][datos_consulta_funcion$anio == ref]

      datos_consulta_funcion[[nombre_var]] <- ifelse(
        datos_consulta_funcion$anio == 2026 & !is.na(valor_ref) & valor_ref != 0,
  ...
  datos_curps <- procedimientos_personas_filtrado %>%
    dplyr::select(
      anio = fecha,
  ...
  for (col in cols_metricas) {
    for (ref in c(2024, 2025)) {
      ...
      valor_2026 <- datos_curps[[col]][datos_curps$anio == 2026]
      valor_ref  <- datos_curps[[col]][datos_curps$anio == ref]

      datos_curps[[nombre_var]] <- ifelse(
        datos_curps$anio == 2026 & !is.na(valor_ref) & valor_ref != 0,
        round((valor_2026 - valor_ref) / valor_ref * 100, 0),
        0
      )
    }
  ...
  pptx <- imprimir_valueboxes_dinamicos(
    ...
    titulo = "Productividad IMSS Bienestar",
    fecha = paste0("Del 1 de enero al ", fecha_portada),
    master = "Tema de Office"
  )

  # Diapo 3 ----------------------------------------------------------------
  layout_historico <- definir_layout_historico(datos_consulta_funcion)

  grafica_consultas <- grafica_planeacion(
    df = datos_consulta_funcion,
    col_valor = "total_consultas",
  ...
  if (!is.na(layout_historico) && layout_historico == "Historico consultas") {
    ...
    pptx <- pptx %>%
      ...
      officer::ph_with(
        ...
        officer::ph_with(value = clock::date_format(fecha_corte,   format = "al %d de %B de cada año",
                                                    locale = clock::clock_locale("es")),
                         location = officer::ph_location_label("fecha"))
  ...
  # Diapo 4 ----------------------------------------------------------------
  serie_mensual_consultas <- historicos_filtrado %>%
    dplyr::mutate(fecha = lubridate::floor_date(fecha, "month")) %>%
    dplyr::filter(!is.na(fecha), !is.na(consulta_total)) %>%
  ...
  fecha_fin_graf <- lubridate::floor_date(fecha_corte, "month") %m-% months(1)
  ...
  pptx <- pptx %>%
    ...
    officer::ph_with(value = clock::date_format(fecha_corte,   format = "al %d de %B de %Y",
                                                locale = clock::clock_locale("es")),
                     location = officer::ph_location_label("fecha"))
  ...
  if (hay_indicador_2026(datos_consulta_funcion, "qx")) {
    ...
    serie_mensual_pq <- historicos_filtrado %>%
      dplyr::mutate(fecha = lubridate::floor_date(fecha, "month")) %>%
      dplyr::filter(!is.na(fecha), !is.na(procedimientos_qx)) %>%
      dplyr::group_by(fecha) %>%
      dplyr::summarise(
    ...
    g_periodos_pq <- grafica_consultas_periodos(
      ...
      titulo = paste0(
        ...
        tolower(clock::date_format(fecha_fin_graf, format="%B %Y", locale = clock::clock_locale("es"))), ")"
      )
    )
  ...
  #     historicos_filtrado = historicos_filtrado,
  #     procedimientos_personas_filtrado = procedimientos_personas_filtrado
  #   )
  # )
  return(pptx)
}
