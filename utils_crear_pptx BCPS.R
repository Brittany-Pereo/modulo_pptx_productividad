...
crear_reporte_productividad <- function(
  ...
  datos_anual <- historicos_filtrado %>%
  ...
  datos_avance <- historicos_filtrado %>%
    dplyr::mutate(
  ...
  datos_consulta_funcion <- procedimientos_personas_filtrado %>%
    dplyr::transmute(
...
# Diapo 3 ----------------------------------------------------------------
datos_consulta_funcion_b <- historicos_filtrado %>% 
    filter(format(fecha, "%m-%d") <= format(fecha_corte, "%m-%d")) %>% 
    mutate(anio = stringr::str_extract(fecha, "\\d{4}")) %>% 
    group_by(anio) %>% 
    summarise(total_consultas = sum(consulta_total),
           consulta_gral = sum(consulta_general),
           consulta_esp = sum(consulta_especialidad),
           qx = sum(procedimientos_qx)) %>% 
    dplyr::left_join(datos_anual %>% 
...
datos_consulta <-       list(
  datos = "C:/Users/brittany.pereo/Downloads/datos_clues_DFIMB001822_2026-04-30.xlsx"|> readxl::read_excel(sheet= "productividad detalle"),
  resumen ="C:/Users/brittany.pereo/Downloads/datos_clues_DFIMB001822_2026-04-30.xlsx" |> readxl::read_excel(skip =7, sheet=1),
  clues_seleccionada = "DFIMB001822"
)
...
presentacion <- crear_reporte_productividad(
  ...
  ruta_master = "data raw/Master presentación.pptx"
  # output = "C:/Users/armando.gonzalez/Downloads/reporte_BCIMB000051.pptx"
)


print(presentacion, target = "C:/Users/brittany.pereo/Downloads/DFIMB001822.pptx")
